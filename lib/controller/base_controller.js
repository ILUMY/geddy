...
controller.BaseController.prototype = new (function () {
  ...
  */
  ...
  this.renderTemplate = function (d, opts, callback) {
    ...
    if(opts.module){
        if(controller.loadedModules.indexOf(opts.module) == -1){
            templater.createTemplate(opts.module);
            controller.loadedModules.push(opts.module);
        }
  ...
  this.setTemplateAndLayout = function (opts) {
      ...
      var viewsLayoutPath = 'app/views/layout/'

      if(opts.module){
          viewsPath = 'app/modules/' + opts.module + '/views/';
      }

    // Set template and layout paths
    if (opts.template) {
      // If template includes full views path just use it
      if (opts.template.match(viewsPath)) {
        ...
        this.template = viewsPath + opts.template;
      }
      // Assume they only included the action, so add the controller path
      else {
        this.template = viewsPath + controllerFilename + '/' + opts.template;
      }
    }
    else {
      this.template = viewsPath + controllerFilename + '/' + actionFilename;
    }

    if (opts.layout) {
      // If layout includes `app/views` just return it
      if (opts.layout.match(viewsPath)) {
      ...
      // and prepend views dir
        ...
        this.layout = viewsPath + opts.layout;
      }
      // Assume they only included the controller, so append it to
      // the layouts path
      else {
        this.layout = viewsLayoutPath + opts.layout;
      }
    }
    else {
      ...
      if (opts.layout === false) {
        ...
        this.layout = viewsLayoutPath + controllerFilename;
      }
    }
